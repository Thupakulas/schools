...
//if($regname != "" && $regmobileno != "" && $regemail != "" && $regpassword != ""  ){
  ...
  $verifyuserinDB=mysqli_query($conn, "select * from adminregister where username='$regname' ");

  $verdyusercount=mysqli_num_rows($verifyuserinDB);

  if($verdyusercount == 0){

      $register=mysqli_query($conn, "INSERT INTO adminregister (username,mobileno,email,password,imageupload) VALUES ('$regname','$regmobileno','$regemail','$regpassword','$file')");
